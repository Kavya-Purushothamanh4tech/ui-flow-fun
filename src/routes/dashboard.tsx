...
import { Bell, MapPin, TrendingUp, ShieldCheck, Clock, CheckCircle2, BadgeIndianRupee, ArrowRight, Camera } from "lucide-react";
  ...
  return (
    <PhoneShell>
      ...
      <div className="bg-gradient-hero text-primary-foreground rounded-b-[36px] px-6 pt-3 pb-24 relative overflow-hidden">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-11 rounded-2xl bg-white/15 backdrop-blur border border-white/20 flex items-center justify-center font-semibold">AM</div>
            <div>
        ...
        <div className="relative mt-6">
          ...
          <p className="text-xs text-white/60 mt-1 flex items-center gap-1">
            <MapPin className="size-3" /> Chennai · Active patrol zone
          </p>
        </div>
      </div>
      ...
      <div className="px-6 -mt-16 grid grid-cols-3 gap-2.5 relative z-10">
        <Stat icon={<Clock className="size-4" />} label="Pending" value="4" tint="bg-warning/15 text-warning-foreground/90" />
        <Stat icon={<CheckCircle2 className="size-4" />} label="Approved" value="18" tint="bg-success/15 text-success" />
        <Stat icon={<ShieldCheck className="size-4" />} label="Reports" value="24" tint="bg-primary/15 text-primary" />
      </div>
      ...
      <div className="px-6 mt-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link to="/submit" className="relative overflow-hidden p-4 rounded-2xl bg-gradient-primary text-primary-foreground shadow-elevated">
          ...
          <Link to="/payouts" className="relative overflow-hidden p-4 rounded-2xl bg-card border border-border shadow-card">
            <div className="size-10 rounded-xl bg-success/15 text-success flex items-center justify-center"><BadgeIndianRupee className="size-5" /></div>
      ...
      <div className="px-6 mt-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/history" className="text-xs font-semibold text-primary">See all</Link>
        </div>
        <div className="space-y-2.5">
          {recent.map((v) => (
            <Link key={v.id} to="/violation/$id" params={{ id: v.id }} className="flex gap-3 p-2.5 rounded-2xl bg-card border border-border shadow-card">
              <img src={v.thumb} alt="" loading="lazy" className="size-16 rounded-xl object-cover" />
