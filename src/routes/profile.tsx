...
import { ChevronRight, Settings, Shield, Bell, HelpCircle, LogOut, Award, Star, Edit3 } from "lucide-react";
  ...
  return (
    <PhoneShell>
      <div className="bg-gradient-hero text-primary-foreground rounded-b-[36px] px-6 pt-3 pb-20 relative overflow-hidden">
        ...
        <div className="relative mt-6 flex flex-col items-center text-center">
          <div className="relative">
            <div className="size-20 rounded-full bg-white/15 border border-white/20 backdrop-blur flex items-center justify-center text-2xl font-bold">AM</div>
          ...
          <h2 className="mt-3 text-lg font-bold">Arjun Mehta</h2>
          <p className="text-xs text-white/70">arjun.m@example.com · +91 98765 43210</p>
      ...
      <div className="px-6 -mt-12 grid grid-cols-3 gap-2.5 relative z-10">
        ...
        <Mini label="Rating" value="4.8" icon={<Star className="size-3 fill-warning text-warning" />} />
      </div>
      ...
      <div className="px-6 mt-6 space-y-2.5">
        ...
        <Link to="/welcome" className="flex items-center justify-center gap-2 rounded-2xl bg-destructive/10 text-destructive p-3.5 font-semibold text-sm">
...
function Mini({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-card border border-border shadow-card p-3 text-center">
      <div className="flex items-center justify-center gap-1 text-xl font-bold">{value}{icon}</div>
      <p className="text-[11px] text-muted-foreground mt-1">{label}</p>
    </div>
  );
}
function Section({ children }: { children: React.ReactNode }) {
  return <div className="rounded-2xl bg-card border border-border shadow-card divide-y divide-border overflow-hidden">{children}</div>;
}
function Row({ icon, label, rightHint }: { icon: React.ReactNode; label: string; rightHint?: string }) {
  return (
    <button className="w-full flex items-center gap-3 p-3.5 hover:bg-accent/40">
      <div className="size-9 rounded-xl bg-accent text-accent-foreground flex items-center justify-center">{icon}</div>
