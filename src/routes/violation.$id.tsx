    ...
    <PhoneShell noPadding>
      ...
      <div className="px-6 pt-5 pb-28 space-y-4">
        <Row
          ...
          label="Location"
          value={v.location}
          sub="40.7527°N, 73.9772°W · GPS verified"
        />
        <Row
          icon={<Clock className="size-5" />}
          label="Timestamp"
          value={v.date}
          sub="Auto-captured · Device clock verified"
        />
        <Row
          icon={<Car className="size-5" />}
          label="Vehicle"
          value={v.vehicle}
          sub="Sedan · Black Toyota Camry"
        />
        <Row
          icon={<User className="size-5" />}
          label="Reported by"
          value={v.reporter ?? "—"}
          sub="Gold Patroller · 18 verified reports"
        />
        ...
        <div className="rounded-2xl bg-card border border-border shadow-card p-4">
          ...
          <p className="mt-2 text-sm leading-relaxed">
            Vehicle crossed the stop line and proceeded through the red signal at the 5th Avenue and
            E 42nd Street intersection. Multiple pedestrians were on the crosswalk at the time.
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-accent/40 border border-border p-4 flex items-center gap-3">
          ...
          <div className="flex-1 text-sm">
            ...
            <p className="text-xs text-muted-foreground">
              Auto-calculated from category & evidence quality
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-background/95 backdrop-blur border-t border-border p-4 flex gap-3 z-40">
        <Button
          ...
          variant="outline"
          className="flex-1 h-12 rounded-2xl border-destructive/30 text-destructive hover:bg-destructive/10 font-semibold"
        >
          <XCircle /> Reject
        </Button>
        <Button
          onClick={() => setConfirm("approve")}
          className="flex-1 h-12 rounded-2xl bg-gradient-success text-success-foreground shadow-elevated font-semibold"
        >
      ...
      {confirm && (
        ...
        >
          ...
          >
            ...
            <div
              className={`size-14 rounded-2xl flex items-center justify-center mx-auto ${confirm === "approve" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
            >
              {confirm === "approve" ? (
