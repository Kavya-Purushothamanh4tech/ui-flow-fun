...
export const Route = createFileRoute("/admin/violations")({ component: AdminViolations });
...
function AdminViolations() {
  ...
  const filtered = violations.filter((v) => {
    if (tab === "Queue") return v.status === "pending" || v.status === "review";
    if (tab === "Approved") return v.status === "approved" || v.status === "paid";
  ...
  return (
    <PhoneShell>
      <div className="px-6 pt-2 pb-2 flex items-center justify-between">
        <Link to="/admin" className="size-10 -ml-2 rounded-full flex items-center justify-center hover:bg-accent">
      ...
      <div className="px-6">
        <div className="relative">
          ...
          <Input className="h-11 pl-10 rounded-2xl bg-card text-sm" placeholder="Search reports, plates, reporters..." />
        </div>
      ...
      <div className="px-6 mt-3 space-y-2.5 pb-8">
        {filtered.map((v) => (
          <Link key={v.id} to="/violation/$id" params={{ id: v.id }} className="flex gap-3 p-2.5 rounded-2xl bg-card border border-border shadow-card">
            <img src={v.thumb} alt="" loading="lazy" className="size-20 rounded-xl object-cover" />
            <div className="flex-1 min-w-0 py-0.5">
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{v.vehicle} · {v.reporter}</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">{v.date}</p>
            </div>
          </Link>
        ))}
      </div>
